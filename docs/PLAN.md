...
- [x] **9. Polish.** Command palette, keyboard-first navigation, themes, motion, empty states.
  ...
  **Outcome.** A command palette on `Cmd/Ctrl-K` filters and runs everything the toolbar and
  ...
  set to system. `rocket-night` extends the same mechanism past flat colour: it overrides the
  surface variables to translucent fills and paints a photograph, a scrim and scanlines on the
  body. The photo stays sharp — no `backdrop-filter` — because blurring it discards the image
  the theme exists to show, so the panels carry their own opacity instead. Anything keying off
  darkness calls `isDark()` rather than comparing against `'dark'`, or a new dark theme
  silently inherits the light editor. A single `Tabs.svelte` now backs both tabbed surfaces with the full ARIA
...
- [x] **10. Packaging.** `electron-builder`, native core as an `extraResource`, per-OS CI,
  ...
  **Outcome.** `desktop/electron-builder.yml` packs `out/` (the electron-vite build) with an
  ...
  `build/entitlements.mac.plist`) and NSIS (Windows). CodeMirror moved to `devDependencies`
  ...
  **Verified locally.** `make package` produced an AppImage (134 MB) and a deb (107 MB), and
  ...
  had spawned the bundled core. Signing/notarization are configured but not exercised here:
  they need an Apple certificate.
